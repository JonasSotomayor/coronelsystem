...
//echo "idusuario=".$idusuario;
	...
	case 'verificar':
	  ...
	  //Hash SHA256 en la contraseña
		...
		$sql="SELECT * FROM detallecomisiondeporte, comisiondeporte	WHERE detallecomisiondeporte.idcomisionDeporte=comisiondeporte.idcomisionDeporte AND usuario='$login' AND password='$clavea' AND cargo='$cargousuario' AND estado='ACTIVO'";
//echo $sql;
		...
		if (isset($fetch))
	    {
	        ...
	        $_SESSION['idusuario']=$fetch->idcomisionDeporte;
	        $_SESSION['razonsocial']=$fetch->nombre;
	        $_SESSION['imagenUsuario']='user.png';
			$_SESSION['ciusuario']=$fetch->ci;
			$_SESSION['usuario']=$fetch->usuario;
			$_SESSION['deporte']=$fetch->deporte;
			$_SESSION['iddeporte']=$fetch->iddeporte;
...
*/
			...
			if ($fetch->cargo=='PRESIDENTE') {
				$_SESSION['AdministradorDeportivo']=1;
			}else if($fetch->cargo=='SECRETARIO'){
				$_SESSION['SecretarioDeportivo']=1;
			}else{
				$_SESSION['TesoreroDeportivo']=1;
			}
